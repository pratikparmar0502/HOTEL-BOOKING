...
//       try {
//         if (typeof setLoading === "function") setLoading(true); // Error check

//         const userData = JSON.parse(localStorage.getItem("user"));
//         const currentUser = userData?.displayName || userData?.email;

//         const response = await api.get("/BookingSystem");
//         const myData = response.data.filter(
//           (b) => b.customerName === currentUser
//         );
...
const Bookings = () => {
  // 1. States Define ki hain (setLoading fix)
  const [bookings, setBookings] = useState([]);
  const [loading, setLoading] = useState(true);

  // 2. Data Fetching Logic with User Filter
  useEffect(() => {
    const fetchMyBookings = async () => {
      try {
        setLoading(true);
        ...
        // Naye user ke liye filter set karna zaroori hai
        const currentUser = userData?.displayName || userData?.email;
        ...
        // ERROR FIX: Check karein ki response.data ek list (Array) hai ya nahi
        const allData = Array.isArray(response.data) ? response.data : [];

        // USER FIX: Sirf current logged-in user ki bookings nikalna
        const myData = allData.filter((b) => b.customerName === currentUser);

        setBookings(myData);
      } catch (err) {
        console.error("Fetch Error:", err);
        setBookings([]); // Error case mein empty list rakhein
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <Container maxWidth="lg" sx={{ py: 8 }}>
      <Box sx={{ mb: 6, textAlign: "center" }}>
        <Typography
          variant="h3"
          sx={{ fontWeight: 900, mb: 2, color: "#1A2027" }}
        >
          My Bookings
        </Typography>
        <Typography variant="body1" color="text.secondary">
          Manage and track your stay history with StayFlow.
        </Typography>
      </Box>
      ...
      {bookings.length > 0 ? (
        <Grid container spacing={3}>
          {bookings.map(
            (
              row // Yahan 'row' define kiya hai [Error Fix]
            ) => (
                ...
                >
                  <Grid container spacing={2} alignItems="center">
                    <Grid item xs={12} md={6}>
                      <Typography
                        variant="h5"
                        sx={{ fontWeight: 800, color: "#1A2027", mb: 1 }}
                      >
                        {row.hotelName} {/* Correct variable usage */}
                      </Typography>
                      <Box
                      ...
                      <Typography
                        variant="caption"
                        sx={{ color: "grey.500", mt: 2, display: "block" }}
                      >
                        Booking ID: #{row._id?.slice(-8).toUpperCase()}
                      </Typography>
                    </Grid>
                    ...
                    >
                      <Typography
                        variant="h4"
                        sx={{ fontWeight: 900, color: "#2E7D32", mb: 1 }}
                      >
                        ₹{row.amount || "0"}
                      </Typography>
                      <Chip
        ...
        <Box sx={{ textAlign: "center", py: 10 }}>
          ...
          <Button
            variant="contained"
            sx={{ borderRadius: "12px", mt: 2, px: 4 }}
